...
Note: the entry-point file is `default.json` (not `.json5`) because Renovate's
GitHub preset resolver only looks for `default.json` (then `renovate.json`)
when no filename is specified — see [renovatebot/renovate#15370][rb15370].
The contents are JSON5 (comments, trailing commas) and Renovate parses them
with the JSON5 parser regardless of extension.
